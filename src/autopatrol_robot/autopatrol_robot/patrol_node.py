...
from autopatrol_interfaces.srv import SpeechText  # 语音合成服务的消息接口
...
class PatrolNode(BasicNavigator):
    def __init__(self, node_name='patrol_node'):
        ...
        # 1.2图像保存参数设置
        self.declare_parameter('img_save_path','/tmp/autopatrol_images')
        ...
        self.img_sub_=self.create_subscription(
            Image, '/camera/image_raw', self.img_callback, 1)
    ...
    def record_img(self):
        if self.latest_img_ is not None:
            ...
            cv2.imwrite(
                os.path.join(
                    self.img_save_path,
                    f'img_{pose.translation.x:3.2f}_{pose.translation.y:3.2f}.png'),
                cv_image
            )
...
# 三、语音播报相关函数
    ...
    def get_current_pose(self):
        while rclpy.ok():
            try:
                result=self.buffer_.lookup_transform('map','base_link',
...
def main():
    ...
    while rclpy.ok():
        ...
        for point in points:
            ...
            patrol.speech_text(f'正在前往{x},{y}目标点')

    # 4.开始导航
            if not patrol.nav_to_pose(target_pose):
                patrol.speech_text(f'前往{x},{y}目标点失败')
                continue

    # 5.记录图像
            patrol.speech_text(f'已经到达目标点{x},{y}，正在准备记录图像')
            patrol.record_img()
            patrol.speech_text(f'图像已记录完成')
